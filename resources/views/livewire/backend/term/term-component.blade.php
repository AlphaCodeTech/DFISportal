                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @foreach ($terms as $term)
                                            <tr>
                                                <td>{{ Str::headline($term->session->name) }}</td>
                                                <td>{{ Str::headline($term->type) }}</td>

                                                <td class="d-flex"
                                                    style="justify-content: space-evenly; padding-right: 0;">
                                                    @can('edit term')
                                                        <a title="edit" wire:click="edit({{ $term }})"
                                                            role="button" class="btn btn-success"><i
                                                                class="fas fa-edit"></i></a>
                                                    @endcan
                                                    <button wire:click="show({{ $term->id }})" role="button"
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">{{ $isEditing ? 'Edit Term' : 'Add New Term' }}</h4>
                                    ...
                                    <form method="POST" wire:submit.prevent="{{ $isEditing ? 'update' : 'store' }}"
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select wire:model.defer='state.session_id'
                                                    class="form-control @error('session_id') is-invalid @enderror">
                                                    <option value="" selected>Select Session</option>
                                                    @foreach ($sessions as $session)
                                                        <option value="{{ $session->id }}"
                                                            {{ optional($selectedTerm)->session?->id == $session->id ? 'selected' : '' }}>
                                                            {{ $session->name }}</option>
                                                    @endforeach
                                                </select>
                                                @error('session_id')
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="d-flex flex-column align-items-center text-center">
                                            ...
                                            <div class="mt-3">
                                                <h4>{{ Str::headline(optional($selectedTerm)->name) ?? '' }}
                                                </h4>
                                    ...
                                    <div class="card-body text-left">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-9 text-secondary">
                                                {{ ucwords(optional($selectedTerm)->session?->name) ?? '' }}
                                            </div>
                                        </div>
                                        <hr>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-sm-9 text-secondary">
                                                {{ ucwords(optional($selectedTerm)->type) ?? '' }}
                                            </div>
                                        </div>
                                        <hr>
